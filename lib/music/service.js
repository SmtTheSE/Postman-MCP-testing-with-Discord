...
import { applyMoodFilters } from './filters.js'
import { resetVotes, clearVotesForGuild, addVote, getVoteCount, countHumansInVoice, votesRequired, isUserInVoiceChannel } from './voteSkip.js'
...
async function startPlaying(guildId, channelId, track) {
  ...
  const mood = getGuildSettings(guildId).mood || 'normal'
  if (mood !== 'normal') {
    applyMoodFilters(guildId, mood).catch((err) => {
      console.warn(`[music] mood filter failed guild=${guildId}:`, err.message)
    })
  }
}
...
export async function setMood(guildId, channelId, mood) {
  ...
  const player = getShoukakuPlayer(guildId)
  if (player) {
    await applyMoodFilters(guildId, nextMood)
  }
