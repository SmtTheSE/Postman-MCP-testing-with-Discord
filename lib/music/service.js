...
import { sfxActiveGuilds, resumeAfterSoundboard, listSoundboardSounds, playSoundboard, addCustomSound, removeCustomSound } from './soundboard.js'
import { fetchLyricsForTrack } from './lyrics.js'
import { listMoodPlaylists, getMoodPlaylist, upsertGuildPlaylist, deleteGuildPlaylist } from './moodPlaylists.js'
...
export function getSoundboardList(guildId) {
  return listSoundboardSounds(guildId)
}

export async function uploadSoundboardClip(guildId, { label, filename, dataBase64, maxMs }) {
  const buffer = Buffer.from(dataBase64, 'base64')
  return addCustomSound(guildId, { label, filename, buffer, maxMs })
}

export async function deleteSoundboardClip(guildId, soundId) {
  return removeCustomSound(guildId, soundId)
}

export function getMoodPlaylistCatalog(guildId) {
  return listMoodPlaylists(guildId)
}

export function saveMoodPlaylist(guildId, payload) {
  return upsertGuildPlaylist(guildId, payload)
}

export function removeMoodPlaylist(guildId, playlistId) {
  deleteGuildPlaylist(guildId, playlistId)
  return listMoodPlaylists(guildId)
}

export async function queueMoodPlaylist(guildId, channelId, playlistId, requestedBy, auth = {}) {
  const playlist = getMoodPlaylist(playlistId, guildId)
  if (!playlist) {
    throw Object.assign(new Error('Unknown mood playlist'), { status: 404, code: 'PLAYLIST_NOT_FOUND' })
  }
  ...
  await setMood(guildId, channelId, playlist.mood)

  let tracks = []
  try {
    tracks = await resolveQuery(playlist.search, requestedBy)
  } catch (e) {
    setPlaybackError(guildId, channelId, 'TRACK_RESOLVE_FAILED', e.message)
    throw e
  }

  const count = Math.min(playlist.trackCount || 5, tracks.length)
  const slice = tracks.slice(0, count)
  if (!slice.length) {
    throw Object.assign(new Error('No tracks found for that mood playlist'), { status: 404, code: 'PLAYLIST_EMPTY' })
  }
  ...
  if (!session?.nowPlaying && !hasPlayer) {
    await startPlaying(guildId, channelId, slice[0])
    if (slice.length > 1) enqueueTracks(guildId, channelId, slice.slice(1))
  } else if (!session?.nowPlaying) {
    await startPlaying(guildId, channelId, slice[0])
    if (slice.length > 1) enqueueTracks(guildId, channelId, slice.slice(1))
  } else {
    enqueueTracks(guildId, channelId, slice)
  }

  pushAction(guildId, channelId, {
    userId: auth.userId || '',
    username: requestedBy,
    action: 'mood playlist',
    detail: playlist.label,
  })
