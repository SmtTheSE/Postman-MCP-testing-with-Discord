...
import { listMoodPlaylists, getMoodPlaylist } from './moodPlaylists.js'
...
export function getMoodPlaylistCatalog() {
  return listMoodPlaylists()
}

export async function queueMoodPlaylist(guildId, channelId, playlistId, requestedBy, auth = {}) {
  const playlist = getMoodPlaylist(playlistId)
  if (!playlist) {
